...
import datetime
import traceback
from scripts.generation.layer_tool_kits import LayerUtils, LayerMatching
import numpy as np
...
def MLA_mut(model, selected_idx=None, mutated_layer_indices=None):
    # add one layer to the end of the model before the dense
    layer_names = list(LayerMatching().layer_concats.keys())
    layer_list = list(LayerMatching().layer_concats.values())
    MLA_model = utils.ModelUtils.model_copy(model, "MLA")
    model_outputs = MLA_model.output
    selected_idx = np.random.randint(len(layer_list)) if selected_idx is None else selected_idx
    selected_layer = layer_list[selected_idx]
    selected_layer_name = layer_names[selected_idx]
    layer_index_range = np.arange(0, len(MLA_model.layers))  # possible inserted indexs: np.arange(0, len(layer_num))
    layer_index_to_insert = -1  # the default layer index is -1
    layer_index_to_insert = np.random.choice(layer_index_range) if mutated_layer_indices is None else np.random.choice(mutated_layer_indices)
    mylogger.info('insert {} after {}'.format(selected_layer_name, MLA_model.layers[layer_index_to_insert].name))

    def new_layer_addition(x, layer):
        x = layer(x)
        layer_input = x
        output_shape = list(layer.output_shape)
        for layer in selected_layer(output_shape):
            y = layer(layer_input)
            layer_input = y
        return layer_input

    new_model = utils.ModelUtils.functional_model_operation(MLA_model, operation={
        MLA_model.layers[layer_index_to_insert].name: new_layer_addition})
    # new_model = keras.models.model_from_json(new_model.to_json(), custom_objects=custom_objects())
    return new_model
...
class MLAMut(object):
    ...
    def mutate(self, model, mutated_layer_indices=None):
        model_json = model.to_json()
        new_model = None
        try_time = 0
        while try_time < 10:
            try_time += 1
            model_copy = copy_model_from_json(model_json)
            layer_indices = None
            mutated_layer_indices = None
            try:
                new_model = MLA_mut(model_copy, selected_idx=layer_indices, mutated_layer_indices=mutated_layer_indices)
                del model_copy
                return new_model
            except Exception:
        ...
        if new_model is None:
            raise ValueError("Fail to add new layers on model: ", model.name)
        return new_model
