...
class Driver(object):
    ...
    def run_lemon_mutants(self):
        lemon_dir = self.output_dir
        exp_lists = os.listdir(lemon_dir)
        for exp in exp_lists:
            main_logger.info(f"Working on exp: {exp}")
            exp_dir = os.path.join(lemon_dir, exp)
            if not os.path.exists(os.path.join(exp_dir, "mutant_history.txt")):
                main_logger.info(f"No Model Generated in EXP: {exp}, SKIP!")
                continue
            from utils.utils import get_files
            model_path_list = get_files(exp_dir, ".h5")
            model_dict = {}
            for model_path in model_path_list:
                model_dict[os.path.basename(model_path)] = model_path
            mutant_list = open(os.path.join(exp_dir, "mutant_history.txt"), "r").read().split("\n")[:-1]
            initial_seed_path = os.path.join(exp_dir, "mut_model", exp+"_origin0.h5")
            self.predict(initial_seed_path, exp+"_origin0.h5")
            for mutant_name in model_dict:
                mutant_path = model_dict[mutant_name]
                self.predict(mutant_path, mutant_name)
