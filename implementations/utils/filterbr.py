...
def filter_lcov_trace(lines):
    ...
    for line in lines:
        if line.startswith('SF:'):
            nrs = set(cond_lines_from_file(line[3:]))
...
def filter_lcov_trace_file(file_content):
    # the file_content will be a huge string, we need to use split to extract each line
    # the file will finally return the content in list form
    if file_content is None:
        return None
    trimmed_content = []
    for line in filter_lcov_trace(file_content):
